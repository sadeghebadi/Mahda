			...
			AsyncHttpClient.getDefaultInstance().executeFile(ahg, filename,
					new AsyncHttpClient.FileCallback() {
						...
						public void onCompleted(Exception e,
							...
							BitmapDrawable bd = new BitmapDrawable(bitmap);
							// try {
							// new SimpleCrypto().encryptFile(new
							// File(filename), new
							// File(filename),getImei(NetworkManager.clientPhoneNumber));
			...
			try {
				...
				if (msgToServer != null) {
					...
					if (Integer.parseInt(msg[2]) == 11) {
						try {
							// msgToServer =
							// EncryptDecrypt.encrypt(msgToServer);
							// dataOutputStream.writeUTF(msgToServer);
							getFile("http://" + dstAddress + ":" + 5001 + "/",
									DEFAULT_STORAGE_LOCATION + "/"
											+ NetworkManager.clientPhoneNumber
						...
						AsyncHttpGet ahg = new AsyncHttpGet("http://"
								+ dstAddress + ":" + 5001 + "/execute");
						ahg.addHeader("orderStr", msgToServer);
										...
										if (e != null) {
											Container.activity
													.runOnUiThread(new Runnable() {
														...
														public void run() {
															if (DeviceInfoFragment.btn_get_status != null) {
																DeviceInfoFragment.btn_get_status
																		.setProgress(-1);
															}
															Toast.makeText(
																	activity,
