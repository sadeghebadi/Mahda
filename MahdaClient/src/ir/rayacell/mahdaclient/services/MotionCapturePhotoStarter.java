...
public class MotionCapturePhotoStarter extends BroadcastReceiver implements
...
//				.findViewById(R.id.sv_camera);
		...
		service.putExtra("orderTime", intent.getExtras().getString("orderTime"));
//		intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		
/*		Container.activity.startService(service);
*/	
		new MotionPictureTimerManager(service);
		//test end
		...
		if (pictureURI != null) {
			...
			if (pictureURIs.size() >= picturesToTake) {
				...
				handler.postDelayed(new Runnable() {
					public void run() {
						camera.autoFocus(MotionCapturePhotoStarter.this);
					}
				}, delay * 1000 /* 100 */);
			}
