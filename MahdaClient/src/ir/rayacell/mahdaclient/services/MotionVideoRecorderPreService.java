 ...
 */
public class MotionVideoRecorderPreService implements SensorEventListener {
	 ...
	 */
	public MotionVideoRecorderPreService(Intent intent) {
		create();
		// tick(orderTime)
		// ontick
		// onfinish pause()
		quality = intent.getExtras().getInt("quality");
	...
	private static final String TAG = "MotionVideoRecorderPreService";
		...
		public void onPreviewFrame(final byte[] data, Camera cam) {
			...
			if (!GlobalData.isPhoneInMotion()) {
				Log.d(TAG, "startThread");
						...
						thread = new DetectionThread(data, size.width,
								size.height);
						thread.start();
			}
		}
				...
				if (img != null && detector.detect(img, width, height)) {
					...
					if (now > (mReferenceTime + Preferences.PICTURE_DELAY)) {
						...
						Intent phService = new Intent(Container.activity,
								MotionVideoRecorderService.class);
						Log.d("Start Capturing", "started by Motion detection");
						phService.putExtra("duration", duration);
						phService.putExtra("quality", quality);
